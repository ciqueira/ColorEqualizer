...
float3 mtl_convert(float3 rgb, int st, bool dir, int cs) {
    if (dir) {
        ...
        if (st==8) return mtl_RGB_to_Spherical(log_rgb);
        return log_rgb;
    }
    ...
    float3 log_rgb=rgb;
    if (st==8) log_rgb=mtl_Spherical_to_RGB(rgb);
    ...
    uint2 id [[ thread_position_in_grid ]])
{
    ...
    if (p_spaceType==8) {
        float3 log_rgb=mtl_normalize_log3(rgb,p_inputCS);
        float peak=max(abs(log_rgb.x),max(abs(log_rgb.y),abs(log_rgb.z)));
        bool invalid=peak<1e-6f;
        float mean=(log_rgb.x+log_rgb.y+log_rgb.z)/3.f;
        float minimum=min(log_rgb.x,min(log_rgb.y,log_rgb.z));
